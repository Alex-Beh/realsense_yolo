...
public:
  // Constructor
  explicit realsense_yolo_detector(
...
private:
  ...
  std::string detection_output_pub, camera_link, marker_array_topic,
      depth_topic;
  ...
  int detection_id_increment_, detection_id_offset_,
      current_detection_id_; // added for multi-sensor use in SPENCER

  // Global variable
  camera_info_vector intrinsic_camera_matrix;
  bool boudingbox_pcl = false;

  message_filters::Subscriber<realsense_yolo::BoundingBoxes3d>
      m_yolo_detection_result_sub;
  message_filters::Subscriber<sensor_msgs::Image> m_depth_img_sub;

  typedef message_filters::sync_policies::ApproximateTime<
      realsense_yolo::BoundingBoxes3d, sensor_msgs::Image>
      SyncPolicy;
  ...
  void
  filterOutUnwantedDetections(const realsense_yolo::BoundingBoxes3d::ConstPtr
                                  &yolo_detection_raw_result,
                              const sensor_msgs::Image::ConstPtr &depth_image);

  void
  camera_infoCallback(const sensor_msgs::CameraInfo::ConstPtr &camera_info_msg);

#ifdef WITH_SPENCER
  spencer_tracking_msgs::DetectedPersons
  fillPeopleMessage(std::vector<bbox_t_3d> result_vec, std::string obj_names,
